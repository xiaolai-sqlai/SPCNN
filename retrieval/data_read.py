...
class ImageFolderPDukeMTMC(Dataset):
    def __init__(self, db_path, transform=None, instance_num=4):
        ...
        id_set = set()
        for class_id in os.listdir(osp.join(db_path, "train", "occluded_body_images")):
            id_set.add(class_id)
        id_list = sorted(list(id_set))

        count = 0
        for class_id in os.listdir(osp.join(db_path, "train", "occluded_body_images")):
            for name in os.listdir(osp.join(db_path, "train", "occluded_body_images", class_id)):
                count += 1
                label = id_list.index(class_id)
                self.label_to_items[label].append([osp.join(db_path, "train", "occluded_body_images", class_id, name), label])
                
            for name in os.listdir(osp.join(db_path, "train", "whole_body_images", class_id)):
                count += 1
                label = id_list.index(class_id)
                self.label_to_items[label].append([osp.join(db_path, "train", "whole_body_images", class_id, name), label])
...
class ImageFolderDeepFashioin(Dataset):
    def __init__(self, db_path, transform=None, instance_num=4):
        self.transform = transform
        self.instance_num = instance_num
        self.label_to_items = defaultdict(list)

        lines = open(osp.join(db_path, "list_eval_partition.txt")).readlines()[2:]
        id_set = set()
        for line in lines:
...
class ImageFolderSOP(Dataset):
    def __init__(self, db_path, transform=None, instance_num=4):
        self.transform = transform
        self.instance_num = instance_num
        self.label_to_items = defaultdict(list)

        lines = open(osp.join(db_path, "Ebay_train.txt")).readlines()[1:]
        id_list = sorted(list(set([lines[idx].strip().split()[1] for idx in range(len(lines))])))
...
if __name__ == "__main__":
    ...
    # dataset = ImageFolderDeepFashioin(db_path=os.path.join(data_dir, 'In-shop'), instance_num=4)
    dataset = ImageFolderPDukeMTMC(db_path=os.path.join(data_dir, 'P-DukeMTMC-reid'), instance_num=4)
    print(len(dataset))
